...
**Fix: ingress UI 502 — pre-bind the listener so it actually accepts IPv4.** v0.33.1 changed the default host to `::` to fix the IPv6-RST surface for `homeassistant.local`-via-mDNS clients, on the assumption that `IPV6_V6ONLY=0` would fall out of the kernel sysctl `net.ipv6.bindv6only=0`. In the HA addon container that assumption does not hold — the resulting Uvicorn listener rejects v4-mapped peers (`127.0.0.1:8901` and `::ffff:127.0.0.1:8901` both get `Connection refused`), even though a freshly-created `socket.socket(AF_INET6)` reports `IPV6_V6ONLY=0`. HA Supervisor's ingress proxy connects to addons by IPv4 hostname, so opening the "HA Ops" panel surfaced as `502 Bad Gateway` / "The app seems to not be ready" while MCP tool calls (proxied via IPv6 on the hassio bridge) kept working.

`__main__.py` now drives the SSE / streamable-http transports through a small runner (`_runner.py`) that pre-binds the listening socket itself: `setsockopt(IPV6_V6ONLY, 0)` *before* `bind`, then `Server.serve(sockets=[sock])` instead of letting uvicorn create its own socket. stdio transport keeps using `mcp.run("stdio")` unchanged. Default host stays at `::` so the b065d91 IPv6-RST fix is preserved; users who set `HA_OPS_HOST` / `host:` in `config.yaml` to a literal IPv4 still get an `AF_INET` socket with no IPv6 logic.
