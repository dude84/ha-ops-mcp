...
- **Checked at startup.** The lifespan now resolves the live HA version (`.HA_VERSION` first, `GET /api/config` as fallback), stores it on the context — `ctx.ha_version` existed but was never populated — and logs a warning if it falls outside the window. Best-effort: an unreachable HA at boot is not a startup failure.
- **Reported by two tools.** `haops_system_info` gains a `compatibility` block, and `haops_tools_check` reports the same under `summary.compatibility`, so the model can see the version relationship without being told about it.
- **New `docs/HA_COMPATIBILITY.md`** — the prose half, and the thing to read after an HA update. Includes a full inventory of the HA API surface we actually depend on: 20 WebSocket command types, 9 REST endpoints, 10 Supervisor endpoints, 10 `.storage` files, 7 recorder tables. HA's breaking-change lists are overwhelmingly integration-level; this is the list to diff them against. Also records version-specific behaviour we've hit (ZHA removal on 2026.7, the Python 3.14 bump, the 2026.8 device-registry split).

**`haops_tools_check` now probes the ZHA WebSocket commands.** The zigbee group only ever read `zigbee.db`, which proves ZHA is installed and nothing about the WS surface — so when HA silently deleted `zha/devices/remove` and `zha/device/remove` (both answer "Unknown command" as of 2026.7), the same class of removal hitting `zha/devices/reconfigure` or `zha/topology/update` would have broken `haops_zha_reconfigure_device` and `haops_zigbee_scan` with no signal. The group now sends `zha/devices/reconfigure` with an all-zero IEEE: a live command rejects it before touching hardware, a removed one answers "Unknown command". `zha/topology/update` is deliberately not probed — it kicks off a 30-60s mesh walk, which is not health-check behaviour. Both commands verified live on 2026.7.4.

**HA Core's port is no longer assumed to be 8123.** HA 2026.8 changes the default for *new* HA OS installs to port 80 (existing installs keep whatever they had). `run.sh` hardcoded `http://homeassistant:8123` on the long-lived-token path, which would have left a fresh 2026.8 install unable to connect. It now probes 8123 then 80 and logs which it found, falling back to 8123 with a warning if neither answers. The Supervisor-token path (`http://supervisor/core`) was never affected.

## 0.54.3

**Fixed a WebSocket reconnect storm on Python 3.11+.** `_is_conn_alive()` tested the connection state with `str(state).endswith("OPEN")`, which relied on the pre-3.11 `IntEnum.__str__` returning `"State.OPEN"`. Python 3.11 changed `IntEnum.__str__` to return the integer, so `str(State.OPEN) == "1"` and the check was **always False** — every `send_command()` tore down and re-authenticated the socket, flooding the log with `connection is dead, reconnecting…` / `authenticated` cycles (up to ~10/s under load) and causing intermittent `HTTP 504` on the ingress UI Health tab (`GET /api/ui/self_check`) plus `close code=1000` clean-closes in HA's own WS health check. Latent since written; exposed when HA Core's bundled Python moved to 3.14. Now compares `state.name == "OPEN"`, which is stable across the `__str__` change and all supported `websockets` versions. Also **guarded the reconnect path with an `asyncio.Lock`** (double-checked) so concurrent `send_command()` callers can't each reconnect and clobber `self._conn`, orphaning each other's socket. Fixes #2.
